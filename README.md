...
Exposes 58 tools for compiling and running projects, navigating/editing/linting source modules, reading/modifying/creating/diffing layouts, exploring libraries, deploying APKs, managing app lifecycle and emulators, debugging via ADB, doing live visual UI verification directly on the device, and slicing/packing/cleaning sprite PNGs — all without leaving your AI coding assistant.
...
| `b4a_add_asset` | Copies a file into the project's `Files\` folder and registers it in the `.b4a` (adds a `File` entry, bumps `NumberOfFiles`, `.bak` first) |
...
| `b4a_layout_add_view` | Adds a Label/Button/EditText/Panel with B4A-correct defaults (name, parent, position/size, one entry per design variant). Creates a `.bak`. |
| `b4a_diff_layout` | Human-readable diff between two layouts (or before/after a change): views added/removed and per-view property changes. |
| `b4a_layout_check_anchors` | Responsive audit — flags hardcoded pixel positions in the layout's DesignerScript that should likely use `%x`/`%y`. |
...
| `b4a_sprite_slice` | Slices a sprite sheet into individual frame PNGs by a cols×rows grid (`baseName_<index>.png`, row-major). |
| `b4a_sprite_pack` | Packs frame PNGs into a single atlas on a uniform grid and writes a JSON metadata sidecar mapping each frame to its `{x, y, w, h}`. |
...
| `b4a_list_emulators` | Lists available Android Virtual Devices (`emulator -list-avds`). |
| `b4a_start_emulator` | Starts an AVD by name (`emulator -avd <name>`), launched detached — poll `b4a_list_devices` until it boots. |
...
│   ├── EmulatorTools.vb     # list/start Android emulators (AVDs)
